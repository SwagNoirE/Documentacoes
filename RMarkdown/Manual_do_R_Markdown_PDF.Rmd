...
\fancyhead[L]{\includegraphics[height=1.7cm]{X:/USID/ESTAGIO_ESTATISTICA/MANUAL/CSS/UFC.png}}
...
\setlength{\singlespacing} # Espaçamento simples
\setlength{\doublespacing} # Duplo espaçamento
\setlength{\onehalfspacing} # 1,5 de espaçamento
...
Os blocos de códigos que aparecerem no texto podem ser roláveis e definidas as dimensões deles. Para que isso ocorra, insira no arquivo de estilo em css: Definindo o scroll:
...
Os conteúdos de tabela ou gráfico pode ser colocadas em abas dentro da página. Isto facilita a organização de forma que os conteúdos estejam visiveis em seções paralelas. Para isso ser possível, coloca-se o comando `{.tabset}` em uma seção de hierarquia superior a dos que serão mostrados.
...
Para que uma imagem possua título, adicione ao chunks de código: `fig.cap = "nome da figura"`. Já para referncia-la no próprio texto, use o código `\@ref(fig:nome_do_chunk)` usando o `bookdown::pdf_document2:`.
...
Os hiperlinks são utilizados quando se referencia informações fora do documento, ou seja, na web. O texto principal normalmente fica em itálico, dentro de um colchete, enquanto o link de fato é colocado posteriormente dentro de um parêntesis. `[*Texto que aparece como link*](link do site)`
...
# Tabelas e figuas {#tabelas-e-figuas}

## Título

A tabela \@ref(tab:A1) gera a imagem \@ref(fig:ggmtcars).

```{r A1, fig.cap = "Exemplo 1"}
